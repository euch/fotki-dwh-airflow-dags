...
from operators.add_metadata_operator import AddMetadataOperator
from operators.bulk_insert_operator import BulkInsertOperator
from operators.create_remote_tree_csv_operator import CreateRemoteTreeCsvOperator
from operators.smb_download_operator import SmbDownloadOperator
...
with (DAG(dag_id=DAG_NAME_DWH_REFRESH, max_active_runs=1, schedule=SCHEDULE_DAILY) as dag):
    ...
    with TaskGroup(group_id='edm') as edm:
        ...
        with TaskGroup(group_id='add_new') as add_new_tg:
            ...
            with TaskGroup(group_id='metadata_archive') as add_metadata_archive:
                add_new(find_missing_metadata_sql='sql/edm/find_missing_metadata_archive.sql',
                        xcom_key='edm.add_new.metadata_archive.find_missing',
                        smb_conn_name=CONN_SMB_ARCHIVE,
                        rp=Variable.get(VAR_RP_ARCHIVE))

            with TaskGroup(group_id='metadata_collection') as add_metadata_collection:
                add_new(find_missing_metadata_sql='sql/edm/find_missing_metadata_collection.sql',
                        xcom_key='edm.add_new.metadata_collection.find_missing',
                        smb_conn_name=CONN_SMB_COLLECTION,
                        rp=Variable.get(VAR_RP_COLLECTION))

            with TaskGroup(group_id='metadata_trash') as add_metadata_trash:
                add_new(find_missing_metadata_sql='sql/edm/find_missing_metadata_trash.sql',
                        xcom_key='edm.add_new.metadata_trash.find_missing',
                        smb_conn_name=CONN_SMB_TRASH,
                        rp=Variable.get(VAR_RP_TRASH))

            SQLExecuteQueryOperator(task_id='tree', conn_id=CONN_POSTGRES, sql='sql/edm/edm_tree_insert_new.sql') >> [
                add_metadata_archive,
                add_metadata_collection,
                add_metadata_trash]
        delete_old >> add_new_tg
